...
const AdminReviewPage: React.FC = () => {
  ...
  const handleApprove = async (id: number) => {
    try {
      ...
      alert(t("admin.reviews.errorValidate"));
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      alert(t("admin.reviews.errorDelete"));
    }
  };
      ...
      <header className={styles.header}>
        <h1>
          ...
          <span className={styles.yellow}>
            {t("admin.reviews.titleHighlight")}
          </span>
        </h1>
            ...
            <div key={rev.id} className={styles.reviewCard}>
              <div className={styles.cardHeader}>
                <div className={styles.authorInfo}>
                  {/* Titre du spectacle mis en avant */}
                  <h4 className={styles.showTitleTarget}>
                    🎭 {rev.showTitle || "Spectacle inconnu"}
                  </h4>
                  <div>
                    <strong className="text-warning">@{rev.authorLogin}</strong>
                    <span className="text-muted ms-2">
                      le {formatDate(rev.createdAt, i18n.language)}
                    </span>
                  </div>
                </div>
