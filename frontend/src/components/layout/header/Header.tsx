    ...
    <header className={styles.header}>
      <div className="container d-flex justify-content-between align-items-center">
        ...
        <nav className={styles.nav}>
          ...
          >
            {t("layout.header.artists") || "Artistes"}6233w
          </NavLink> */}
          <NavLink
            to="/about"
            className={({ isActive }) =>
          ...
          {/* ESPACE PRODUCTEUR - Modération des avis ajoutée ici */}
          {user && user.role === "producer" && (
            <div
              className={styles.dropdown}
              onMouseEnter={() =>
                handleEnter(setIsProducerOpen, producerTimeout)
              }
              onMouseLeave={() =>
                handleLeave(setIsProducerOpen, producerTimeout)
              }
            >
              <button
                type="button"
              ...
              {isProducerOpen && (
                <div className={styles.dropdownMenu}>
                  ...
                  <NavLink
                    ...
                    className={styles.dropdownItem}
                  >
                    ⭐ {t("layout.header.reviewsModeration")}
                  </NavLink>
                </div>
              )}
            </div>
          ...
          {/* ESPACE ADMIN - Modération des avis retirée ici */}
          {user && user.role === "admin" && (
            <div
              className={styles.dropdown}
              onMouseEnter={() => handleEnter(setIsAdminOpen, adminTimeout)}
              onMouseLeave={() => handleLeave(setIsAdminOpen, adminTimeout)}
            >
              ...
              {isAdminOpen && (
                <div className={styles.dropdownMenu}>
                  <NavLink to="/admin" end className={styles.dropdownItem}>
                    📊 {t("layout.header.dashboardGlobal")}
                  </NavLink>
                  <NavLink to="/admin/users" className={styles.dropdownItem}>
                    👥 {t("layout.header.usersManagement")}
                  </NavLink>
                  <NavLink to="/admin/shows" className={styles.dropdownItem}>
                    🎭 {t("layout.header.showsModeration")}
                  </NavLink>
                  {/* Le lien des avis a été supprimé d'ici */}
                  <NavLink
                    to="/admin/locations"
                    className={styles.dropdownItem}
                  >
                    📍 {t("layout.header.locationsManagement")}
                  </NavLink>
                </div>
              )}
            </div>
        ...
        <div className={styles.actions}>
          ...
          {user ? (
            ...
            <div className={styles.authButtons}>
              <Link to="/become-producer" className={styles.registerBtn}>
                {t("layout.header.becomeProducer")}
              </Link>
